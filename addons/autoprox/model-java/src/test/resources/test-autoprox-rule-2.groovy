...
class AnotherTestRule extends AbstractAutoProxRule
{
    boolean matches( String named ){
        named.startsWith( "OSS-" )
    }

    RemoteRepository createRemoteRepository( String named )
        throws MalformedURLException
    {
        def match = (named =~ /OSS-(.+)/)[0]
        return new RemoteRepository( name: named, url: "https://oss.sonatype.org/content/repositories/${match[1]}/" )
    }
