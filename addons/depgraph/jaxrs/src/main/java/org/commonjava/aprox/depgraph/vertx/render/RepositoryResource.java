    ...
    implements RequestHandler
{
    ...
    public void getUrlMap( final Buffer body, final HttpServerRequest request )
    {
        try
        {
            ...
            final String json = controller.getUrlMap( body.getString( 0, body.length() ), baseUri, new JaxRsUriFormatter() );
    ...
    public void getDownloadLog( final Buffer body, final HttpServerRequest request )
    {
        try
        {
            ...
            final String downlog = controller.getDownloadLog( body.getString( 0, body.length() ), baseUri, new JaxRsUriFormatter() );
            if ( downlog == null )
            {
                Respond.to( request )
                       .noContent()
